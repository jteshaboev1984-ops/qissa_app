# Королевство семи дорог — World Bible V1

> Status: canonical companion to owner-approved Story V1 «Праздник мужества»  
> Date: 2026-09-23  
> Scope: story continuity + **owner-approved visual canon V1** for future stories and illustrations  
> Rule: HARD CANON and APPROVED VISUAL CANON must not change silently. Future intentional redesigns require a new version and an explicit canon delta.

## 1. Why this file exists

«Праздник мужества» is the first approved story in this world. Future stories, prompts, illustrations, covers, episode previews and memory records must use the same established facts.

The literary text should stay readable and should not repeat full appearance descriptions every time. Detailed recurring visual information belongs here and in character/location reference assets.

There are three levels:

- **HARD CANON** — established and must remain consistent.
- **VISUAL CANON V1** — locked design details for illustrations once approved.
- **OPEN / NOT YET CANON** — intentionally undecided; do not invent a permanent answer inside a later story without an explicit canon update.

---

# 2. World identity

## HARD CANON

**World name:** Королевство семи дорог.

**Known concept:** seven important roads connect the kingdom. The annual Festival of Courage takes place in the capital.

**Period / technology:** pre-industrial storybook world based on horses, roads, handwritten letters, wax seals, sabres, guards, road posts and walled cities. No modern vehicles, firearms, phones, electricity, modern signage or contemporary clothing.

**Magic status:** no magic has been established in V1. Future stories must not casually treat magic, magical creatures or supernatural powers as already normal parts of the world. If magic is ever introduced, that is a separate canon decision.

**Cultural / architectural direction:** Central Asian / Silk Road-inspired fantasy rather than generic medieval Western Europe. It is a fictional kingdom, not a direct reconstruction of one historical state.

Use visual language such as:
- broad courtyards;
- arched gates;
- carved wood;
- stone and warm plaster walls;
- restrained glazed-tile accents;
- textiles and riding clothes inspired by Central Asian silhouettes;
- practical horse tack and travel gear.

Avoid turning the world into:
- a European Gothic castle setting;
- a modern city;
- an Arabian-fantasy cliché;
- a direct copy of a real historical dynasty.

---

# 3. Story 1 — fixed chronology

The following events are HARD CANON.

1. The Festival of Courage happens once a year.
2. Sixteen children take part in the selection shown in V1.
3. Temur and Samira did not know each other before that day.
4. The selection includes running, archery and preparing a horse for travel.
5. Temur and Samira become the two young royal couriers for the final challenge.
6. Both receive identical sealed royal letters.
7. Both must ride to the same city — Zaran — give the letter to its ruler, receive a separate sealed reply and return it to the king.
8. The stated rule is that the winner is the one who first **honestly** completes the mission and returns the reply.
9. Temur receives Shamol; Samira receives Bulut.
10. Both receive a road book.
11. A fallen tree blocks the main road.
12. Samira knows from the road book that the upper path is open and shorter.
13. Samira lies to Temur: «Через лес. Эта закрыта.»
14. Temur trusts her and rides through the Eastern Forest.
15. Samira takes the shorter upper route.
16. In the Eastern Forest Temur notices the bandits' sign on a tree.
17. Bandits block his route and demand his horse and bag.
18. The same sign appears on one bandit's leather bag.
19. Temur uses his short sabre to cut a rope blocking the passage. He does **not** attack the bandits.
20. Temur escapes because of quick thinking, riding skill and Shamol.
21. Shamol jumps a ditch during the escape.
22. After the escape Temur and Shamol have established real mutual trust.
23. Samira reaches a river after the previous night's rain.
24. She uses the road book to recognize that the lower ford is unsafe and finds the old stone crossing upstream.
25. Samira reaches Zaran first.
26. The ruler of Zaran records that Samira arrived first and that her royal seal was intact.
27. Temur reaches Zaran later.
28. Temur tells the ruler and city guard about the forest bandits and reproduces their sign from memory.
29. Zaran's guard says other travelers have previously mentioned people carrying the same mark, but the attacks had not yet been clearly connected.
30. The ruler includes this information in Temur's sealed reply to the king.
31. Samira leaves Zaran first and expects to win.
32. On the return route she discovers a rain-damaged landslide/road collapse hidden beyond a bend.
33. She realizes Temur may approach too fast to see the danger.
34. Samira gives up the chance to preserve her lead and stays to warn him.
35. She climbs higher so Temur can see her before the bend.
36. Wet ground collapses beneath her and she ends up on a ledge below the road.
37. Bulut remains safely above and neighs.
38. Temur sees/hears Bulut, stops in time and learns Samira is below.
39. Temur does not attempt a reckless solo rescue. He rides to the nearby road post and brings adults and proper ropes.
40. Samira is rescued.
41. Afterward Samira voluntarily admits that the upper route at the first fork had been open and that she lied to Temur because she wanted to win.
42. Temur does not instantly erase the wrongdoing. He says he is glad she is alive but will still be angry.
43. Samira asks why Temur did not simply ride on.
44. Temur answers: «Тогда я бы всё равно знал, что уехал.»
45. They return to the capital almost together.
46. Both reply seals are intact.
47. The king reads Samira's reply first and confirms that she reached Zaran first.
48. Temur's reply alerts the crown to the bandit pattern and sign.
49. Samira tells the full truth about the first fork herself.
50. The king makes clear that the Festival tests more than speed.
51. Samira is the first courier to reach Zaran.
52. Both Temur and Samira are judged worthy to become **юные бахадуры царства** (young bahadurs of the realm).
53. The bandit problem is **not solved** in Story 1.
54. By the end of Story 1 the king and captain of the royal guard know the bandits' sign.
55. The bandits and their sign are an unresolved hook for later stories.

### Story 1 title / ceremony canon

- Official title after Story 1: **юные бахадуры царства**; singular: **бахадур царства**.
- This supersedes the earlier working phrase “young royal knight” in post-V1 canon and app-facing Story 1 text. The frozen owner-approved V1 snapshot remains historical and is not silently rewritten.
- Temur and Samira remain **standing** side by side for the ceremony.
- There is no kneeling and the ruler does not touch their shoulders with a blade.
- A senior warrior/captain brings two broad ceremonial belts with metal buckles and two sabres in scabbards.
- The belts are worn over the existing travel clothes; the ruler presents each sheathed sabre **hilt-first**.
- The final ceremony image should show the children upright with the belts and sheathed sabres, with the capital square, realm banners, Shamol and Bulut, and townspeople behind them.

Do not later rewrite these facts as if:
- Temur won the race outright;
- Samira never lied;
- Temur defeated the bandits in combat;
- the bandits were captured;
- the two children were old friends before the festival;
- they rode to different cities;
- there was only one letter/reply;
- the king already knew everything about the bandits before Temur's report.

---

# 4. Character canon

## Temur — HARD CANON

**Role:** one of the two young royal couriers in Story 1; becomes a **бахадур царства** (bahadur of the realm).

**Origin:** a small town in the east of the kingdom.

**Established personality:**
- quick;
- curious;
- observant;
- notices unusual details;
- willing to act fast;
- does not confuse bravery with pointless fighting;
- accepts help from adults when a rescue requires it;
- capable of anger without cruelty;
- values the fact that he can live with his own decision.

**Important competence established in V1:**
- riding;
- using a short sabre as a tool;
- remembering visual signs;
- keeping calm under pressure;
- making a quick escape plan.

**Relationship with Samira after Story 1:** emerging mutual respect, but not instant perfect friendship. He knows she lied to him; she admitted it herself. Future stories may develop trust gradually.

**Relationship with Shamol after Story 1:** strong trust formed through the forest escape.

## Temur — APPROVED VISUAL CANON V1

Use this exact base design for recurring illustrations unless a later canon version explicitly changes it:

- approximate age: 12;
- Central Asian boy;
- lean, athletic child build;
- warm medium skin tone;
- dark brown, almost black straight hair;
- short hair, slightly tousled at the front;
- dark brown eyes;
- expressive dark eyebrows;
- no facial hair;
- no adult-looking proportions;
- primary riding outfit: deep teal knee-length riding chapan/tunic with narrow muted-gold edging;
- light beige trousers;
- dark brown riding boots;
- brown leather belt;
- small dark-brown leather travel satchel;
- short, gently curved sabre in a plain dark leather scabbard with a brass-toned guard;
- no crown;
- no heavy armor;
- no European knight plate armor.

**Story 1 damage continuity:** after the forest escape, his **right sleeve/forearm area is visibly torn and dusty**. This remains visible in Zaran, on the return road and at the final ceremony until he changes clothes after the story.

---

## Samira — HARD CANON

**Role:** one of the two young royal couriers in Story 1; reaches Zaran first; becomes a **бахадур царства** (bahadur of the realm).

**Origin:** the south of the kingdom.

**Established personality:**
- observant;
- thinks before acting;
- reads instructions carefully;
- competitive;
- capable of making a selfish decision;
- capable of recognizing that decision as wrong;
- chooses to warn Temur even when it costs her advantage;
- admits the original lie without being forced to invent an excuse;
- practical under pressure.

**Important competence established in V1:**
- riding;
- using the road book;
- reading road and crossing signs;
- route planning;
- judging environmental danger.

**Relationship with Temur after Story 1:** emerging mutual respect; trust is not magically complete.

**Relationship with Bulut after Story 1:** calm, familiar partnership; Bulut is attentive and has his own comic personality.

## Samira — APPROVED VISUAL CANON V1

- approximate age: 12;
- Central Asian girl;
- lean, athletic child build;
- warm medium skin tone;
- dark brown eyes;
- long very dark brown hair;
- hair worn in **one thick braid** during travel;
- no adult makeup;
- child proportions, not a young-adult glamour design;
- primary riding outfit: deep burgundy/maroon knee-length riding chapan/tunic with narrow muted-gold edging;
- cream/light beige trousers;
- dark brown riding boots;
- brown leather belt;
- small dark-brown leather travel satchel;
- no crown;
- no plate armor.

**Story 1 damage continuity:** after the landslide rescue, the lower hem, knees and one side of her outfit have visible damp earth/mud. Her braid can be slightly loosened, but it does not become a completely different hairstyle.

---

# 5. Horse canon

## Shamol — HARD CANON

**Name:** Шамол.

**Temur's horse.**

**Established temperament:**
- energetic;
- restless at first;
- quick;
- responsive;
- brave enough to follow Temur through the forest escape;
- the bond with Temur strengthens visibly during Story 1.

## Shamol — APPROVED VISUAL CANON V1

- young horse;
- dark bay, reading almost black at a distance;
- black mane and tail;
- **small narrow white star on the forehead**;
- no white socks;
- athletic but not huge warhorse proportions;
- dark-brown leather tack;
- deep teal saddle blanket matching Temur's visual family.

The white forehead star is a locked identifier. Never remove it, enlarge it into a blaze or give it to Bulut.

---

## Bulut — HARD CANON

**Name:** Булут.

**Samira's horse.**

**Established temperament:**
- outwardly calm;
- patient;
- food-motivated / gently comic;
- attentive to danger;
- refuses to step into the unsafe river;
- stays above when Samira falls and neighs loudly.

## Bulut — APPROVED VISUAL CANON V1

- young horse;
- light dapple-gray coat;
- darker gray mane and tail;
- **no forehead star or blaze**;
- calm, soft facial expression;
- athletic but not heavy;
- dark-brown leather tack;
- deep burgundy saddle blanket matching Samira's visual family.

Do not make Bulut pure white in one image and dark gray in another. His base is light dapple gray.

---

# 6. Recurring adult characters

## The King — HARD CANON

- ruler of the Kingdom of Seven Roads;
- personally oversees the final Festival of Courage challenge;
- values honesty, judgment and responsibility rather than raw speed;
- receives and opens the sealed replies;
- by the end of Story 1 knows about the bandit sign.

## The King — APPROVED VISUAL CANON V1

- Central Asian man, approximately late 40s;
- composed rather than intimidating;
- dark hair with some gray;
- short, neat beard with visible gray;
- royal deep-blue long chapan/robe with restrained gold embroidery;
- simple gold circlet/crown with **seven small raised points** as a recurring Seven Roads motif;
- no oversized jeweled European crown;
- no full battle armor during the festival.

---

## Captain of the Royal Guard — HARD CANON

- present at the final return;
- reads the report about the bandits;
- takes Temur's remembered sign seriously;
- the bandit investigation becomes adult responsibility.

## Captain — APPROVED VISUAL CANON V1

- Central Asian man, approximately 40s;
- practical military bearing;
- dark green long guard coat;
- leather belt and sword;
- small silver Seven Roads clasp/badge;
- no full face helmet in dialogue scenes.

---

## Ruler of Zaran — HARD CANON

- receives each courier separately;
- records arrival order and intact seals;
- questions Temur about the bandits;
- brings in the local guard captain;
- sends the bandit information to the king.

## Ruler of Zaran — APPROVED VISUAL CANON V1

- older adult, approximately 50s;
- practical civic ruler rather than monarch;
- warm ochre / brown formal robe;
- dark cap or simple civic headwear;
- no crown.

---

# 7. The bandits

## HARD CANON

- operate in/around the Eastern Forest;
- use a recognizable sign;
- can block paths and prepare simple traps/obstructions;
- attempt to take Temur's horse and bag;
- are not defeated or captured in Story 1;
- other travelers have mentioned people with the same mark;
- the crown and Zaran authorities now suspect connected attacks.

No canon answer yet for:
- exact number of bandits in the wider group;
- leader identity;
- hideout;
- motive beyond robbery;
- whether they serve a larger antagonist;
- whether they operate on more than one of the Seven Roads.

Do not invent these as established history until a future story decides them.

## Bandit sign — HARD + APPROVED VISUAL CANON V1

**Asset ID:** `symbol_bandits_v1`

Exact form:
- a simple crescent shape open to the **right**;
- crossed by one short diagonal slash running from **lower left to upper right**;
- rough hand-cut / hand-drawn quality;
- no text;
- monochrome when carved or inked.

Known appearances in Story 1:
1. carved into tree bark in the Eastern Forest;
2. marked on a bandit's leather bag;
3. reproduced by Temur for the ruler of Zaran;
4. reproduced again for the king/royal guard.

Never:
- flip the crescent direction;
- change the slash direction;
- turn it into a star-and-crescent;
- add letters;
- replace it with a skull;
- use a different gang symbol for the same bandits.

---

# 8. Important props

## Road book

**Asset ID:** `prop_road_book_v1`

HARD CANON:
- both couriers receive one;
- contains Seven Roads routes, old paths, crossings and road signs;
- Samira studies it more carefully;
- it helps her at the first fork and at the flooded ford.

VISUAL CANON V1:
- small hand-held travel book;
- brown weathered leather cover;
- embossed simple seven-spoke road motif on the cover;
- cream pages;
- hand-drawn maps and black-ink road symbols;
- compact enough to fit in the travel satchel.

Do not render it as a giant atlas, modern printed guidebook, glowing magic book or smartphone-like object.

---

## Royal outbound letters

**Asset ID:** `prop_royal_letter_v1`

- two identical copies;
- cream parchment;
- rolled/folded to fit a small leather case;
- red wax seal;
- same royal seal design on both;
- seal intact until the ruler of Zaran opens it.

## Leather letter cases

**Asset ID:** `prop_letter_case_v1`

- small dark-brown leather case/futlar;
- practical strap/closure;
- one carried by each child;
- visually identical except normal wear.

## Return letters from Zaran

**Asset ID:** `prop_zaran_reply_v1`

- cream parchment;
- local ruler's seal distinct from the royal red seal;
- **deep blue wax** for visual differentiation;
- each reply is separately sealed;
- both seals remain intact until the king opens them.

## Temur's sabre

**Asset ID:** `prop_temur_sabre_v1`

- short, child-manageable curved sabre;
- functional but simple;
- dark leather scabbard;
- brass-toned guard;
- not oversized;
- used in Story 1 to cut the blocking rope, not to wound a person.

---

# 9. Location canon

## Capital — Festival square

**Asset ID:** `loc_capital_festival_square_v1`

Recurring anchors:
- broad main square;
- royal palace at one side with wide steps;
- seven long festival flags;
- palace/guard architecture in Central Asian fantasy style;
- royal stables accessible from the square area;
- city gate visible/understood as the departure/return point.

For Story 1, do not redraw the square as a completely different city between opening and ending illustrations.

## Royal stable

**Asset ID:** `loc_royal_stable_v1`

- spacious warm-stone / timber stable;
- practical stalls;
- tack room feel;
- same visual family as the palace, not a modern stable.

## Eastern Forest

**Asset ID:** `loc_eastern_forest_v1`

- dense mature forest;
- narrow riding paths;
- enough undergrowth to conceal people;
- old trees suitable for carved route/gang marks;
- ditch appears on the escape path;
- no enchanted glow or fantasy monsters established.

## Zaran

**Asset ID:** `loc_zaran_v1`

- walled provincial city;
- greener, more open visual feeling than the capital;
- warm stone/plaster gate;
- civic residence/hall for the local ruler;
- not a second royal palace;
- surrounding meadows/agricultural green space may justify the name visually.

## River crossing

**Asset ID:** `loc_zaran_river_crossing_v1`

- lower ford made dangerous by recent rain;
- water muddy and fast;
- small stone marker/sign by the bank;
- old stone crossing lies upstream.

## Return-road landslide

**Asset ID:** `loc_return_landslide_v1`

- rain-damaged road along a slope;
- dangerous section hidden just beyond a bend;
- part of the road collapsed downward;
- nearby safer high ground from which Samira tries to signal;
- nearby road post exists within a short ride.

---

# 10. Relationship state after Story 1

This state must be remembered before Story 2 begins.

## Temur ↔ Samira

- started as strangers;
- became direct competitors;
- Samira deliberately misled Temur at the fork;
- Samira later sacrificed her lead to warn him;
- Samira voluntarily confessed the original lie;
- Temur helped rescue her by bringing adults;
- he did not pretend the lie no longer mattered;
- there is new respect and the beginning of trust;
- they are **not yet written as lifelong best friends** unless later stories earn that relationship.

## Temur ↔ Shamol

- initial testing/uncertainty;
- forest escape creates clear trust;
- Shamol responds to Temur quickly under pressure.

## Samira ↔ Bulut

- comfortable partnership;
- Bulut's calmness and appetite can supply light humor;
- he also shows good hazard instinct.

## Crown ↔ bandit threat

- concern has begun;
- sign is known;
- investigation can become a later plotline;
- threat remains unresolved.

---

# 11. Visual continuity rules for all illustrations

These rules are mandatory once final art production starts.

### A. Never rely on prose prompt alone for recurring characters

Each recurring person/horse needs one approved reference sheet. Future illustrations should use:
- the reference image itself where the image system supports references;
- the exact asset ID;
- the locked visual description from this file.

A new prompt must not freely redesign a recurring character.

### B. Stable identifiers

For every recurring visual asset, preserve:
- face shape;
- age;
- skin tone;
- hair color and hairstyle;
- eye color;
- body proportions;
- base outfit silhouette;
- horse coat and markings;
- tack;
- recurring prop design;
- gang symbol geometry.

### C. Outfit continuity inside one story

During the same journey:
- Temur stays in the teal riding outfit;
- Samira stays in the burgundy riding outfit;
- Shamol keeps teal saddle cloth;
- Bulut keeps burgundy saddle cloth.

Do not change costume color because an image model produced a prettier result.

### D. Damage continuity

Illustrations must remember events:
- before Eastern Forest escape: Temur's outfit is intact;
- after escape: right sleeve/forearm is torn/dusty;
- before landslide: Samira's outfit is relatively clean;
- after rescue: mud/damp-earth marks remain through the return and ceremony.

### E. Prop continuity

If a character has the letter case or road book in one sequence, its appearance must match earlier images.

The road book is not magically larger, differently colored or re-bound later.

### F. Left/right continuity

If a fixed physical feature or object placement is introduced in a reference sheet, do not mirror it randomly in later art.

Important example:
- the bandit crescent opens right;
- its slash rises lower-left → upper-right.

### G. Scene continuity

The same location must retain major anchors across images:
- palace shape and steps;
- city gate;
- road direction;
- tree/forest visual family;
- Zaran gate/civic hall.

Camera angle may change. Architecture identity may not.

### H. Age safety / child depiction

Temur and Samira must always visibly read as children around 12, not as 16–20-year-old fantasy heroes.

No sexualized clothing, poses or adult glamour treatment.

### I. Violence depiction

Story tension may be shown, but illustrations should prefer:
- pursuit;
- blocked road;
- rope cutting;
- horse jump;
- rescue;
over explicit weapon-on-person violence.

Temur's sabre scene should visually show him cutting the rope, not attacking a bandit.

### J. Art style lock

The approved master style reference is **QISSA_MASTER_REFERENCE_user_selected.png**, previously selected by the owner. Working style label: **seven_roads_painterly_v1**. The master image is the authoritative style reference; descriptive text is secondary.

Before producing a full set of final story illustrations:
1. choose one approved master style;
2. create reference sheets for Temur, Samira, Shamol, Bulut, King and key props;
3. approve one capital establishing shot and one outdoor road/forest shot;
4. lock that style version;
5. produce the remaining images from those references.

Do not mix watercolor, 3D animation, photorealism and comic-book rendering in the same story unless a later explicit art-direction decision says so.

---

# 12. Recommended canonical asset IDs

Use stable IDs in future prompts, manifests and database records:

- `char_temur_v1`
- `char_samira_v1`
- `horse_shamol_v1`
- `horse_bulut_v1`
- `char_king_seven_roads_v1`
- `char_royal_guard_captain_v1`
- `char_zaran_ruler_v1`
- `symbol_bandits_v1`
- `prop_road_book_v1`
- `prop_royal_letter_v1`
- `prop_letter_case_v1`
- `prop_zaran_reply_v1`
- `prop_temur_sabre_v1`
- `loc_capital_festival_square_v1`
- `loc_royal_stable_v1`
- `loc_eastern_forest_v1`
- `loc_zaran_v1`
- `loc_zaran_river_crossing_v1`
- `loc_return_landslide_v1`

These IDs now refer to the owner-approved V1 visual designs. If a visual design is intentionally redesigned later, create `_v2`; never silently overwrite what `_v1` means.

---

# 13. Suggested illustration continuity state for Story 1

For image generation, track the story state rather than only the scene text.

## State S0 — Festival / selection
- Temur clean, teal outfit;
- Samira clean, burgundy outfit;
- no bahadur status yet;
- Shamol/Bulut introduced;
- letters not yet carried during selection.

## State S1 — Departure
- both clean;
- letter cases carried;
- road books carried;
- horses fully tacked.

## State S2 — Split routes
- Samira clean;
- Temur clean before deep forest;
- fallen tree and two alternative routes established.

## State S3 — Forest escape
- Temur transitions from clean to torn/dusty right sleeve;
- Shamol remains dark bay with small white star;
- bandit sign exact geometry;
- sabre cuts rope.

## State S4 — Samira river crossing
- Samira still clean;
- Bulut light dapple gray;
- flooded ford + stone marker + upstream stone crossing;
- road book visible if scene requires it.

## State S5 — Zaran
- Samira clean and arrives first;
- Temur later arrives with torn/dusty sleeve;
- same city gate/civic hall in both arrival illustrations.

## State S6 — Landslide
- Samira becomes muddy after fall/rescue;
- Temur remains torn/dusty;
- both horses keep same tack and markings.

## State S7 — Return / bahadur ceremony
- Temur still carries forest wear;
- Samira still carries landslide mud unless the story explicitly gives time to change, which V1 does not;
- both are still visually the same children;
- Temur and Samira remain standing during the ceremony; no kneeling and no blade-to-shoulder gesture;
- each receives a broad ceremonial belt with a metal buckle and a sheathed sabre presented hilt-first;
- the ceremonial sabre is separate from Temur's travel sabre if both are visible.

---

# 14. OPEN / NOT YET CANON

Do not permanently invent these yet:

- name of the capital;
- names of Temur's or Samira's families;
- exact family backgrounds;
- exact map geometry of all Seven Roads;
- names and functions of the other six major destinations;
- history of the kingdom;
- whether the world contains magic;
- bandit leader;
- bandit hideout;
- true meaning/origin of the bandit sign;
- whether the bandits are connected to a larger enemy;
- default illustration count for future stories (Story 1 Interactive V3 has its own approved production target).

These should be decided when they become necessary, then added to the canon deliberately.

---

## Story 1 Interactive V3 illustration status

For «Праздник мужества» Interactive V3 specifically:
- rendering style is already **APPROVED VISUAL CANON V1**: `seven_roads_painterly_v1`;
- the current production illustration map targets **27 assets total**: 1 cover, 18 shared narrative illustrations and 8 choice illustrations;
- this count is Story-1-specific and does not establish a mandatory count for future stories;
- the authoritative placement/count source is `2026-09-23_prazdnik_muzhestva_v3_illustration_map.md`.

The six saved approved images are visual/reference canon. They are not automatically assumed to be the final production illustration for every slot unless the slot explicitly reuses one.

---

# 15. Core rule for future stories

Before writing Story 2 or any later episode, load this world bible and the latest canon delta.

Before generating an illustration, load:
1. this world bible;
2. the exact scene state;
3. the approved reference assets for every recurring character/horse/prop/location in the image.

A future story may **develop** a character or reveal new facts. It must not silently contradict established facts.


---

# 16. Canon update — Zaran

**Decision:** the first named city of the world is **Заран (Zaran)**.

The frozen owner-approved Story V1 may still contain the earlier working name «Луговой»; do not silently rewrite that preserved snapshot. All subsequent story versions, world canon, illustrations and future stories must use **Заран**.

## Name / visual meaning

The root **«зар»** should be felt visually as a warm gold / precious-light motif, but not as a literal city made of gold.

Recurring Zaran visual language:
- warm golden-beige stone and plaster in sunlight;
- restrained brass / bronze / muted-gold accents on gates and civic details;
- sunlit wheat or dry-gold grasses around greener cultivated land;
- warm late-afternoon glow that can make the city seem to “hold” light;
- turquoise/blue accents may appear sparingly as contrast, consistent with the Seven Roads world.

Avoid:
- covering every surface in shiny gold;
- palace-like luxury that makes Zaran look richer than the capital;
- generic European castle towers;
- turning «зар» into a magical glow unless magic is later introduced into canon.

Suggested literary description for Story V2:

> За последним холмом открылся Заран.
>
> Город стоял среди широких полей, и под солнцем его светлые стены казались почти золотистыми. Отсюда и начиналось то самое ощущение «зара»: не блеск золота, а тёплый цвет камня, сухих трав и вечернего света.
>
> Над главными воротами поднимались две невысокие башни. За стенами виднелись сады, плоские крыши домов и высокая башня городской ратуши. На воротах и деревянных балконах поблёскивали небольшие медные детали.
>
> После шумной столицы Заран казался спокойным и просторным.

This description is a **draft for Story V2**, not a retroactive change to the frozen V1 file.


---

# 17. Owner-approved image reference set — 2026-09-23

The owner reviewed the generated reference images and approved the overall character, horse, environment and rendering direction.

Persistent reference folder:
`/QISSA/visual_refs/seven_roads_v1/`

Approved files:
- `QISSA_MASTER_REFERENCE_user_selected.png` — master style reference selected earlier by the owner.
- `01_heroes_horses_approved.png` — primary appearance reference for Temur, Samira, Shamol and Bulut.
- `02_capital_festival_approved.png` — capital / Festival of Courage atmosphere and architectural language.
- `03_eastern_forest_escape_approved.png` — Eastern Forest atmosphere, Temur + Shamol action language, bandit-sign usage.
- `04_zaran_arrival_approved.png` — Zaran's golden-warm civic architecture and landscape language.
- `05_landslide_rescue_approved.png` — return-road terrain, landslide scale and post-incident character continuity.

## What the approval locks

The approval locks:
- the painterly illustrated storybook rendering language;
- Central Asian / Silk Road fantasy art direction;
- Temur's and Samira's broad face/hair/age/outfit identities;
- Shamol's and Bulut's coat identities and visual contrast;
- turquoise/blue family for Temur and Shamol tack;
- burgundy/red family for Samira and Bulut tack;
- warm pale-stone + turquoise-accent architectural family for the kingdom;
- Zaran's golden-beige / sunlit visual identity;
- the Eastern Forest's dense mature-tree visual family;
- the landslide-road geography and overall mountain/valley language.

## What the approval does NOT automatically lock

Image-generation incidental details are not canon unless separately stated in this Bible. In particular:
- random one-off symbols on background banners;
- exact crowd faces;
- exact number of towers/windows;
- exact flower placement;
- accidental jewelry or ornamental variations;
- any text rendered inside the generated image;
- mirrored decorative details that conflict with written canon;
- a decorative headpiece reading as a royal crown.

For Samira, the approved visual may include a narrow decorative embroidered headband. It is **not** a royal crown or princess tiara.

For Temur, the approved cap is a practical embroidered Central Asian-style riding cap and can remain part of `char_temur_v1`.

The written canon takes priority over accidental generated-image artifacts.


---

# 18. Canon delta from Story V2 editorial pass — 2026-09-23

These details are now canonical for future story continuity once Story V2 is promoted:

## Temur's lost road book

**Object:** Temur's personal road book from the Festival of Courage.

**Status after Story 1:** LOST / UNRESOLVED.

**Exact loss location:**
- Eastern Forest;
- during the escape from the bandits;
- on the narrow escape trail immediately after the rope barrier Temur cuts with his sabre;
- the book falls from his satchel and lodges between the exposed roots of a **large forked oak**;
- this is **before** Shamol's later jump across the ditch.

**Important:** Temur sees where it falls but cannot safely turn back because the bandits are still pursuing him.

**Future-story rule:** the book may be recovered, discovered by another character, or found by the bandits in a later story, but none of those outcomes is canon yet. Do not silently return the book to Temur between stories.

## First-fork behavior

Temur does not blindly accept Samira's claim. He notices that the upper path does not visibly look blocked, but chooses to trust Samira because she is the one who studied the road book.

Immediately after accepting her answer, Temur rides off quickly **before Samira has put her book away or prepared to start**, reinforcing his impulsive/fast-moving character.

## Trust state after Samira's confession

After Samira confesses the lie, Temur does not instantly restore full trust.

When Samira later proposes another route from the road book, Temur pauses, asks to see the book himself, verifies the entry, and only then agrees.

This is the first visible consequence of the broken trust.

**Relationship state remains:** respect and trust are rebuilding, not fully restored.

## King speech direction

The king's final speech should remain substantial enough to feel ceremonial, but should be grounded in the children's concrete actions rather than a long abstract checklist of virtues.

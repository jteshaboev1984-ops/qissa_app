...
const contexts = actualChoices.map((choice) => {
  ...
  assert.equal(afterChoice.choiceHistory[0].resolution_text, choice.resolution_text, 'Client-visible choice bridge must be preserved verbatim')
  const context = normalizeStoryRequest({ selections, seriesState: afterChoice })
  const normalizedBridge = choice.resolution_text.replaceAll('Malika', '{{HERO}}')
  assert.ok(context)
  ...
  assert.equal(context.choiceHistory[0].resolution_text, normalizedBridge, 'Server must tokenize canonical hero in memory before model input')
  ...
  assert.equal(request.confirmed_choice_bridge.resolution_text, normalizedBridge)
  assert.equal(request.output_contract.choices, 'exactly 0')
  assert.match(system, /Never restage, expand, paraphrase or replay/iu)
  return context
})

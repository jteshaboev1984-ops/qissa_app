...
const sourcePaths = {
  ...
  openingClosings: join(root, 'supabase/functions/story-generate/storyWorldOpeningClosings.ts'),
...
const transpile = (source) => ts.transpileModule(source, {
  ...
  .replace(/['"]\.\/storyWorldOpeningClosings\.ts['"]/g, "'./storyWorldOpeningClosings.mjs'")
...
try {
  const [
    ...
    readFile(sourcePaths.openingClosings, 'utf8'),
  ...
  await Promise.all([
    ...
    writeFile(join(temp, 'storyWorldOpeningClosings.mjs'), transpile(openingClosingsSource)),

...
assert.match(orchestrator, /generateStoryBlueprint\(openAiApiKey, architectModel, context, architectTimeoutMs\)/, 'Architect must use allocated rather than hardcoded timeout')
assert.match(orchestrator, /generateStoryNarration\(openAiApiKey, narratorModel, context, blueprint, '', narrationTimeoutMs\)/, 'Narrator must use allocated timeout')

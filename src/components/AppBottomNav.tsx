...
export function AppBottomNav({ language, tab, onTab }: { language: Language; tab: AppTab; onTab: (tab: AppTab) => void }) {
  ...
  return (
    <nav
      ...
      aria-label={t(language, 'app.title')}
    >
      <div className="grid grid-cols-3 gap-1.5">
        {tabs.map((item) => {
          ...
          return (
            <button
              key={item}
              className={`min-h-11 flex items-center justify-center gap-1.5 rounded-full px-3 py-2.5 text-xs font-bold transition active:scale-[0.98] ${
                active

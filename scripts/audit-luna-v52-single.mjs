...
const seriesId = 'audit-luna-7a8cc03f-ca04-4056-819d-88e791c68ddf'
...
const invokeState = async (payload) => {
  ...
  if (!response.ok) throw new Error(`story-state ${response.status}: ${parsed.text.slice(0, 400)}`)
  return parsed.body
}
...
for (let attempt = 1; attempt <= 40; attempt += 1) {
  ...
  if (runtime === 'enabled') {
    if (probe.response.status !== 403 || probe.body?.error !== 'privacy_consent_required') {
      throw new Error(`Runtime readiness returned ${probe.response.status}: ${probe.text.slice(0, 300)}`)
    }
    ready = true
...
if (paid.response.status !== 200) throw new Error(`Paid Story request returned ${paid.response.status}: ${paid.text.slice(0, 600)}`)
if (metadata.source !== 'openai-structured' || episode?.generationSource !== 'openai-structured') {
  throw new Error(`Paid request did not publish provider story: ${paid.text.slice(0, 1600)}`)
}
...
const pastAfter = new RegExp(`${token}${tokenBoundary}${optionalModifiers}\\s+${unicodeWordStart}${genderedPastWord}${unicodeWordEnd}`, 'iu')
const pastBefore = new RegExp(`${unicodeWordStart}${genderedPastWord}${unicodeWordEnd}${optionalModifiers}\\s+${token}${tokenBoundary}`, 'iu')
if (preposition.test(heroGrammarText) || pastAfter.test(heroGrammarText) || pastBefore.test(heroGrammarText)) throw new Error('Published RU hero grammar still requires rewrite')

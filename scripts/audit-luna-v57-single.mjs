...
const installationId = '9208b55b-81fd-477b-af6f-f8de0d94f5d2'
const installationAuth = randomBytes(32).toString('hex')
const seriesId = 'audit-luna-v57-5caf01ca-c313-41cc-bb28-d83c26090690'
...
// Exactly one consented Story request; no request-level retry.
...
if (words < 360 || words > 500) throw new Error(`Story word count out of range: ${words}`)

// Editorial regression: final prose must not duplicate the two structured choices.
const stop = new Set(['можно','нужно','чтобы','вместе','помочь','герой','героиня','потом','сначала'])
...
const repeatsMenu = episode.choices.every((choice) => {
  const words = sig(choice.text)
  const overlap = [...words].filter((word) => finalWords.has(word)).length
  return words.size >= 3 && overlap >= Math.max(3, Math.ceil(words.size * 0.35))
})
if (repeatsMenu) throw new Error(`story_text duplicated the structured choice menu: ${finalParagraph}`)

// Editorial regression: preview is child-facing, never pipeline language.
...
// Russian HERO token must be safe to substitute without case/agreement corruption.

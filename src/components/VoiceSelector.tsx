...
export function VoiceSelector({ language, selectedVoiceId, onSelect }: VoiceSelectorProps) {
  return (
    <div className="space-y-2 rounded-xl border border-slate-700 bg-slate-900/75 p-3">
      <p className="text-xs font-semibold text-slate-300">{t(language, 'listen.change_voice')}</p>
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {voicePresets.map((preset) => (
          <button
            ...
            className={`rounded-xl border px-3 py-3 text-left text-sm leading-snug transition ${

...
const seriesId = `audit-luna-v59-${randomUUID()}`
...
const readerPreferences = {
  ...
  showTextWithAudio: true, audioOnlyNightMode: true, voicePresetId: 'neutral_storyteller', defaultPlaybackMode: 'read',
}
const initialSeriesState = {
  id: seriesId, childProfileId: installationId, sessionId, sessionIndex: 1,
  stylePackId: 'cozy_forest', mainCharacter: heroName, recurringCharacters: [],
  lastEpisodeSummary: '', activeArc: '', relationshipState: {}, choiceHistory: [], canonState: {}, episodeCount: 0,
}
...
const assertProvider = (result, label) => {
  ...
  if (metadata.source !== 'openai-structured' || episode?.generationSource !== 'openai-structured') throw new Error(`${label}: provider story not published`)
  if (metadata.runtimeAi !== 'enabled' || metadata.pipeline !== 'split-v1') throw new Error(`${label}: bad runtime/pipeline metadata`)
  if ([metadata.architectModel, metadata.narratorModel, metadata.narratorModelUsed, metadata.safetyModel].some((value) => value !== 'gpt-5.6-luna')) throw new Error(`${label}: unexpected model metadata`)
  if (metadata.escalationModel !== 'disabled' || metadata.escalationUsed !== 'false') throw new Error(`${label}: Sol escalation used/configured`)
...
// Readiness probe is provider-free and waits until the operator enables the runtime flag.
let ready = false
...
// Paid call 1: Episode 1. No request-level retry.
...
if (episode1.nextEpisodePreview?.toLocaleLowerCase().includes(' или ')) throw new Error('episode1 preview is branch-recapping')
...
// Paid call 2: Episode 2 continuation after selected child choice. No request-level retry.
...
if (/^(утром|на следующее утро)\b/iu.test(episode2.story_text.trim())) throw new Error('episode2 reset to next morning before resolution')
...
// Persist the completed continuation snapshot, read it back, then fully delete audit data.
...
console.log('CONTINUATION_ACCEPTANCE_RESULT', JSON.stringify({
  episode1: { metadata: first.metadata, wordCount: episode1Words, episode: episode1 },
  selectedChoice,
  continuationMemory: {
    canonKeys: Object.keys(continuationState.canonState),
    relationshipKeys: Object.keys(continuationState.relationshipState),
  },
  episode2: { metadata: second.metadata, wordCount: episode2Words, episode: episode2 },
  persistence,
}, null, 2))

...
const mode = readFileSync('audit/TEMP-v97-capture-trigger.txt', 'utf8').trim()
const ids = JSON.parse(readFileSync('audit/TEMP-v97-capture-ids.json', 'utf8'))
const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
const assert = (condition, message) => { if (!condition) throw new Error(message) }
for (const key of ['capture_id', 'installation_id', 'series_uuid']) assert(uuid.test(ids[key] || ''), `invalid ${key}`)
if (mode === 'DRY') { console.log('DRY PASS: v97 runner valid; provider calls 0'); process.exit(0) }
...
assert(endpoint && anon, 'missing endpoint/key')
const payload = {
  ...
  seriesState: { id: `qissa-synthetic-diagnostic-${ids.series_uuid}`, sessionId: `qissa-synthetic-diagnostic-${ids.series_uuid}`, sessionIndex: 1, mainCharacter: 'Malika', recurringCharacters: [], lastEpisodeSummary: '', activeArc: '', relationshipState: {}, choiceHistory: [], canonState: {}, episodeCount: 0 },
...
let response
try {
  response = await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json', apikey: anon, authorization: `Bearer ${anon}`, origin: 'https://jteshaboev1984-ops.github.io', 'x-qissa-synthetic-diagnostic-id': ids.capture_id }, body: JSON.stringify(payload), signal: controller.signal })
...
assert(body && typeof body === 'object', 'response body not JSON')
...
console.log('LIVE COMPLETE: exactly one synthetic v97 E1 POST; raw story withheld from logs')

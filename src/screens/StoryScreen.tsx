...
export function StoryScreen({ language, episode, readerPreferences, onReaderPreferencesChange, onChoiceSelected, onContinueNextEpisode, isChoiceSavedForCurrentEpisode = false }: StoryScreenProps) {
  ...
  const readerStyle = getReaderTextStyle(readerPreferences)
  const readerCardClass = getReaderThemeClass(readerPreferences.theme)
  ...
  return (
    <section className="space-y-4 rounded-3xl bg-white p-5 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1">
        <button onClick={() => handleViewModeChange('read')} className={`rounded-xl px-4 py-3 text-sm font-medium ${viewMode === 'read' ? 'bg-white shadow-sm' : ''}`}>{t(language, 'story.read_mode')}</button>
        <button onClick={() => handleViewModeChange('listen')} className={`rounded-xl px-4 py-3 text-sm font-medium ${viewMode === 'listen' ? 'bg-white shadow-sm' : ''}`}>{t(language, 'story.listen_mode')}</button>
      </div>
      {viewMode === 'read' ? (
        <>
          <div className="flex justify-end">
            <button type="button" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium" onClick={() => setShowReaderSettings((v) => !v)}>
              Aa
            </button>
          </div>
          {showReaderSettings && <ReaderSettingsPanel language={language} preferences={readerPreferences} onChange={onReaderPreferencesChange} onClose={() => setShowReaderSettings(false)} />}
          <article className={`rounded-2xl p-4 ${readerCardClass}`} style={readerStyle}>{episode.story_text}</article>
        </>
      ) : (
        <ListeningScene
          language={language}
